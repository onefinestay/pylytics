...
class TableBuilder(object):
    ...
    def _get_data(self, source_name):
        ...
        if source_name == None:
            ...
            source['cols_types'] = {n: t for n, t in zip(cols_names,
                                                        cols_types)}
        ...
        # For an extra source
            ...
            if len(cols_names) <= 1:
                ...
                source['cols_types'] = {n: t for n, t in zip(cols_names[1:],
                                                             cols_types[1:])}
                try:
                    source['join_on'] = self.main_source['cols_names'].index(
                        source['join_on'])
                except Exception as e:
                    raise UnknownJoinOnColumnError(source['join_on'],
                                                   source_name)
    ...
    def _append_result_row(self, row, matches):
        ...
        result_row = []
        checks = [
            matches[match][0] != None or self.sources[match]['outer_join'] for
            match in matches]
        ...
        if all(checks):
            ...
            result_row = [
                result_row_unordered[f] for f in self.result_cols_names]
        ...
        for row in self.main_source['data']:
            ...
            for source_name, source_data in self.sources.items():
                ...
                try:
                    ...
                    matches[source_name] = (None, ) * len(
                        source_data['cols_names'])
