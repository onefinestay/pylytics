...
class GroupBy(object):
    ...
    def __init__(self, data_input, group_by, cols=None, dims=[]):
        ...
        self.functions_corresp = {
            self._get_col_id(e): self.functions[k] for k, l in group_by[
                'aggregate'].items() for e in l}
    ...
    def _get_output_cols(self):
        output_indices = sorted(
            set(self.indexes + self.dims + self.functions_corresp.keys()))
    ...
    def _get_col_id(self, col_name):
        if self.input_cols is None:
            ...
            if self.cols_dict is None:  # Initializing cols_dict if necessary
                self.cols_dict = {name: i for i, name in enumerate(
                    self.input_cols)}
        ...
        for key, group in self.groups.items():
            ...
            for col, values in enumerate(zip(*group)):
                ...
                if col in self.indexes or col in self.dims:
                    ...
                    output_row.append(
                        self.functions_corresp[col](cleaned_values))
